...
class FutureStreamBuilderExample extends StatelessWidget {
  const FutureStreamBuilderExample({Key? key}) : super(key: key);
...
class _HomePageState extends State<HomePage> {
  ...
  var _page = 0;
  Future<bool>? _bottomSheetSuccess;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return StreamBuilder<Color>(
          initialData: ColorsProvider.current,
          stream: ColorsProvider.colors,
          builder: (context, snapshot) {
...
class ColorsProvider {
  static Color current = Colors.indigo;

  static Stream<Color> get colors async* {
    while (true) {
      await Future.delayed(const Duration(seconds: 3));
      final colors = <Color>[...Colors.accents, ...Colors.primaries];
